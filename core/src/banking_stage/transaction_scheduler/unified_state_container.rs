...
use {
    super::{unified_priority_id::UnifiedPriorityId, unified_scheduling_unit::UnifiedSchedulingUnit, transaction_state::TransactionState, bundle_state::BundleState},
    crate::banking_stage::scheduler_messages::{MaxAge, TransactionId, BundleId},
...
/// 2. Inserted into `UnifiedStateContainer` by `BankingStage`
/// 3. Popped in priority-order by scheduler, and transitioned to `Pending` state
/// 4. Processed by `ConsumeWorker`
///    a. If consumed, remove `Pending` state from the `UnifiedStateContainer`
...
pub(crate) struct UnifiedStateContainer<Tx: TransactionWithMeta> {
    capacity: usize,
    priority_queue: MinMaxHeap<UnifiedPriorityId>,
    id_to_transaction_state: Slab<TransactionState<Tx>>,
    id_to_bundle_state: Slab<BundleState>,
}

#[cfg_attr(feature = "dev-context-only-utils", qualifiers(pub))]
pub(crate) trait StateContainer<Tx: TransactionWithMeta> {
    /// Create a new `UnifiedStateContainer` with the given capacity.
    ...
    fn pop(&mut self) -> Option<UnifiedPriorityId>;

    /// Get mutable transaction state by id.
    fn get_mut_transaction_state(&mut self, id: TransactionId)
        -> Option<&mut TransactionState<Tx>>;

    /// Get mutable bundle state by id.
    fn get_mut_bundle_state(&mut self, id: BundleId)
        -> Option<&mut BundleState>;
    ...
    fn retry_transaction(&mut self, transaction_id: TransactionId, transaction: Tx) {
        ...
        let priority_id = UnifiedPriorityId::new(transaction_state.priority(), UnifiedSchedulingUnit::Transaction(transaction_id));
...
impl<Tx: TransactionWithMeta> StateContainer<Tx> for UnifiedStateContainer<Tx> {
    fn with_capacity(capacity: usize) -> Self {
        Self {
            capacity,
            ...
            id_to_bundle_state: Slab::with_capacity(capacity + EXTRA_CAPACITY),
        }
    }
    ...
    fn get_mut_bundle_state(&mut self, id: BundleId) -> Option<&mut BundleState> {
        self.id_to_bundle_state.get_mut(id)
    }
    ...
    fn push_ids_into_queue(
        ...
        for _ in 0..num_dropped {
            ...
            match priority_id.id {
                ...
                UnifiedSchedulingUnit::Bundle(id) => {
    ...
    fn clear(&mut self) {
        ...
        self.id_to_bundle_state.clear();
    }
}

impl<Tx: TransactionWithMeta> UnifiedStateContainer<Tx> {
    ...
    pub(crate) fn insert_new_transaction(
        ...
        let priority_id = {
            ...
            UnifiedPriorityId::new(priority, UnifiedSchedulingUnit::Transaction(transaction_id))
        };

        self.push_ids_into_queue(std::iter::once(priority_id)) > 0
    }

    /// Insert a new bundle into the container's queues and maps.
    /// Returns `true` if a packet was dropped due to capacity limits.
    #[cfg_attr(feature = "dev-context-only-utils", qualifiers(pub))]
    pub(crate) fn insert_new_bundle(&mut self, bundle: SanitizedBundle, max_age: MaxAge, priority: u64, cost: u64) -> bool {
        let priority_id = {
                assert!(self.id_to_bundle_state.len() < self.id_to_bundle_state.capacity());

            let entry = self.id_to_bundle_state.vacant_entry();
            let bundle_id = entry.key();
            entry.insert(BundleState::new(bundle, max_age, priority, cost));
            UnifiedPriorityId::new(priority, UnifiedSchedulingUnit::Bundle(bundle_id))
        };
...
/// A wrapper around `UnifiedStateContainer` that allows re-uses
...
pub struct TransactionViewStateContainer {
    inner: UnifiedStateContainer<RuntimeTransactionView>,
...
impl StateContainer<RuntimeTransactionView> for TransactionViewStateContainer {
    fn with_capacity(capacity: usize) -> Self {
        let inner = UnifiedStateContainer::with_capacity(capacity);
    ...
    #[inline]
    fn get_mut_bundle_state(&mut self,id:BundleId) -> Option< &mut BundleState> {
        self.inner.get_mut_bundle_state(id)
    }
    ...
    #[inline]
    fn retry_transaction(&mut self,transaction_id:TransactionId,transaction:RuntimeTransactionView) {
        self.inner.retry_transaction(transaction_id,transaction);
    }
...
mod tests {
    ...
    #[test]
    fn test_is_empty() {
        let mut container = UnifiedStateContainer::with_capacity(1);
    ...
    fn test_get_mut_transaction_state() {
        let mut container = UnifiedStateContainer::with_capacity(5);
        push_to_container(&mut container, 5);
